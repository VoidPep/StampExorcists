...
@export var WAVES: Array[Dictionary] = [
	...
	{ "count": 1, "interval": 0.0, "is_boss_wave": true, "escort_count": 4, "escort_interval": 0.4 }
]
...
func _start_wave(wave_index: int) -> void:
	...
	if wave_data.get("is_boss_wave", false):
		_spawn_single(boss_scene, wave_data, true)
		var escort_count: int = wave_data.get("escort_count", 3)
		var escort_interval: float = wave_data.get("escort_interval", 0.4)
		for i in escort_count:
			await get_tree().create_timer(escort_interval * i + 0.2).timeout
			_spawn_single(enemy_scene, wave_data, false)
	else:
...
func _spawn_single(scene: PackedScene, wave_data: Dictionary, is_boss: bool) -> void:
...
func _on_enemy_died() -> void:
	active_enemies -= 1
	if active_enemies > 0 or not room_active:
		return
	room_active = false
